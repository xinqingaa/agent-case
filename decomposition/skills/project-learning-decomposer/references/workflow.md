...
2. Record project ID, name, audience, source revision when available, authorization level, exclusions, and prohibited actions.
...
Only perform actions allowed by L4 or L5. Capture runtime versions before dependency installation. Separate harmless read-only commands from operations that can write data or alter the environment.
